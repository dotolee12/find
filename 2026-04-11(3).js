...
const PHOTO_POPUP_SIZE = 200;
const PHOTO_THUMB_SIZE = 40;
const PHOTO_JPEG_QUALITY = 0.7;
const PHOTO_DB_NAME = "giloa-photo-db";
...
let isPhotoDbAvailable = "indexedDB" in window;
let currentPhotoMarkerSize = null;
...
const fogOffscreenCanvas = document.createElement("canvas");
const fogOffscreenCtx = fogOffscreenCanvas.getContext("2d");

const ui = {
    distVal: document.getElementById("dist-val"),
    todayDistVal: document.getElementById("today-dist-val"),
    memoryCountVal: document.getElementById("memory-count-val"),
    photoCountVal: document.getElementById("photo-count-val"),
    ...
    progDistCur: document.getElementById("prog-dist-cur"),
    progDistBar: document.getElementById("prog-dist-bar"),
    progDistNext: document.getElementById("prog-dist-next"),
    progMemCur: document.getElementById("prog-mem-cur"),
    progMemBar: document.getElementById("prog-mem-bar"),
    progMemNext: document.getElementById("prog-mem-next"),
    progPhotoCur: document.getElementById("prog-photo-cur"),
    progPhotoBar: document.getElementById("prog-photo-bar"),
    progPhotoNext: document.getElementById("prog-photo-next"),
    memoryList: document.getElementById("memory-list-container"),
    photoList: document.getElementById("photo-list-container"),
    sidebar: document.getElementById("sidebar"),
    sidebarOverlay: document.getElementById("sidebar-overlay"),
    gpxSavedList: document.getElementById("gpx-saved-list"),
    gpxImportStatus: document.getElementById("gpx-import-status"),
    gpxExportName: document.getElementById("gpx-export-name")
};

// ✅ 수정: 캔버스를 지도 컨테이너에 직접 붙임
...
function resizeCanvas() {
    ...
    fogOffscreenCanvas.width = w;
    fogOffscreenCanvas.height = h;
    scheduleRender();
}
...
function openPhotoDb() {
    if (!isPhotoDbAvailable) return Promise.resolve(null);
    if (!photoDbPromise) {
        photoDbPromise = new Promise((resolve, reject) => {
            ...
            console.error("photo db open failed", error);
            isPhotoDbAvailable = false;
            return null;
        });
    }
...
async function withPhotoStore(mode, handler) {
    ...
    return new Promise((resolve, reject) => {
        ...
        console.error("photo db transaction failed", error);
        return null;
    });
}
...
async function savePhotoAsset(photo) {
    return withPhotoStore("readwrite", async store => {
...
async function loadPhotoAsset(id) {
    return withPhotoStore("readonly", store => requestToPromise(store.get(id)));
}

async function deletePhotoAsset(id) {
    return withPhotoStore("readwrite", async store => {
...
async function hydratePhotosFromDb() {
    const hydrated = await Promise.all(photos.map(async photo => {
        if (typeof photo.photo === "string" && typeof photo.thumb === "string") return photo;
        const asset = await loadPhotoAsset(photo.id);
        if (!asset) return null;
        return { ...photo, photo: asset.photo, thumb: asset.thumb };
    }));
    photos = hydrated.filter(photo => photo && typeof photo.photo === "string" && typeof photo.thumb === "string");
}

async function migrateInlinePhotoAssets() {
    const legacyPhotos = photos.filter(photo => typeof photo.photo === "string" && typeof photo.thumb === "string");
    if (!legacyPhotos.length) return;
    await Promise.all(legacyPhotos.map(photo => savePhotoAsset(photo)));
    persistState();
}

function createPhotoIcon(src, size) {
    return L.divIcon({
        className: "photo-marker",
        html: `<img src="${src}" style="width:${size}px;height:${size}px;object-fit:cover;border-radius:6px;border:2px solid #fff;" />`,
        iconSize: [size, size],
...
function renderFog() {
    ...
    for (const [alpha, drawFns] of buckets) {
        fogOffscreenCtx.clearRect(0, 0, w, h);
        fogOffscreenCtx.fillStyle   = `rgba(0,0,0,${alpha})`;
        fogOffscreenCtx.strokeStyle = `rgba(0,0,0,${alpha})`;
        fogOffscreenCtx.lineWidth   = radius * 2;
        fogOffscreenCtx.lineCap     = "round";
        fogOffscreenCtx.lineJoin    = "round";
        drawFns.forEach(fn => fn(fogOffscreenCtx));

        fogCtx.save();
        fogCtx.globalCompositeOperation = "destination-out";
        fogCtx.drawImage(fogOffscreenCanvas, 0, 0);
        fogCtx.restore();
    }
}
...
function updatePhotoMarkerSizes() {
    ...
    if (size === currentPhotoMarkerSize) return;
    currentPhotoMarkerSize = size;
    photoMarkers.forEach(marker => {
        if (marker._photoData) marker.setIcon(createPhotoIcon(marker._photoData.thumb, size));
    });
}
...
function updateHud() {
    ...
    if (ui.hudTitleText) ui.hudTitleText.textContent = current.title;
    if (ui.hudLevelNum)  ui.hudLevelNum.textContent  = current.level;
    if (ui.progDistCur) ui.progDistCur.textContent = distKm.toFixed(2) + " km";
    if (ui.progDistBar && ui.progDistNext) {
        if (!nextRow) { ui.progDistBar.style.width = "100%"; ui.progDistNext.textContent = "최고 레벨 달성!"; }
        else {
            const pct = nextRow.distKm > current.distKm ? Math.min(100, ((distKm - current.distKm) / (nextRow.distKm - current.distKm)) * 100) : 100;
            ui.progDistBar.style.width = pct.toFixed(1) + "%";
            const remain = Math.max(0, nextRow.distKm - distKm);
            ui.progDistNext.textContent = remain > 0.01 ? `다음까지 ${remain.toFixed(1)}km` : "조건 충족!";
        }
    }
    if (ui.progMemCur) ui.progMemCur.textContent = memCount + " 개";
    if (ui.progMemBar && ui.progMemNext) {
        if (!nextRow || nextRow.memories === 0) { ui.progMemBar.style.width = "100%"; ui.progMemNext.textContent = nextRow ? "조건 없음" : "최고!"; }
        else {
            ...
            ui.progMemBar.style.width = pct.toFixed(1) + "%";
            const remain = Math.max(0, nextRow.memories - memCount);
            ui.progMemNext.textContent = remain > 0 ? `다음까지 ${remain}개` : "조건 충족!";
        }
    }
    if (ui.progPhotoCur) ui.progPhotoCur.textContent = photoCount + " 개";
    if (ui.progPhotoBar && ui.progPhotoNext) {
        if (!nextRow || nextRow.photos === 0) { ui.progPhotoBar.style.width = "100%"; ui.progPhotoNext.textContent = nextRow ? "조건 없음" : "최고!"; }
        else {
            const pct = nextRow.photos > current.photos ? Math.min(100, ((photoCount - current.photos) / (nextRow.photos - current.photos)) * 100) : 100;
            ui.progPhotoBar.style.width = pct.toFixed(1) + "%";
            const remain = Math.max(0, nextRow.photos - photoCount);
            ui.progPhotoNext.textContent = remain > 0 ? `다음까지 ${remain}개` : "조건 충족!";
        }
    }
}
...
function updateStats() {
    ...
    if (ui.distVal)        ui.distVal.innerHTML        = `${(totalDistance / 1000).toFixed(2)}<span>km</span>`;
    if (ui.todayDistVal)   ui.todayDistVal.innerHTML   = `${(todayDist / 1000).toFixed(2)}<span>km</span>`;
    if (ui.memoryCountVal) ui.memoryCountVal.innerHTML = `${memories.length}<span>개</span>`;
    if (ui.photoCountVal)  ui.photoCountVal.innerHTML  = `${photos.length}<span>개</span>`;
    updateHud();
}

function toggleHud() {
    isHudExpanded = !isHudExpanded;
    ui.hud.classList.toggle("expanded", isHudExpanded);
    ui.controls.classList.toggle("hud-open", isHudExpanded);
    ui.helpBtn.classList.toggle("hud-open", isHudExpanded);
    if (isHudExpanded) {
...
function handleHudOutsideClick(event) {
    if (!ui.hud.contains(event.target)) {
        isHudExpanded = false;
        ui.hud.classList.remove("expanded");
        ui.controls.classList.remove("hud-open");
        ui.helpBtn.classList.remove("hud-open");
        document.removeEventListener("click", handleHudOutsideClick);
    }
}
...
function findPhotoMarker(id) { return photoMarkers.get(id) || null; }
...
function exportGpx() {
    ...
    const nameInput = ui.gpxExportName.value.trim();
...
${trkpts}
    ...
    ui.gpxExportName.value = "";
    ui.gpxImportStatus.textContent = `✓ "${name}" 저장 완료`;
}
...
function updateGpxSavedList() {
    const container = ui.gpxSavedList;
    if (!container) return;
    const saves = loadGpxSaves();
...
function importGpxFile(event) {
    ...
    const statusEl = ui.gpxImportStatus;
    ...
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({
            ...
            photos: photos.map(p => ({
                ...
                photo: isPhotoDbAvailable ? undefined : p.photo,
                thumb: isPhotoDbAvailable ? undefined : p.thumb,
                time: p.time, dateString: p.dateString, timeString: p.timeString
            })),
            totalDistance
        }));
    ...
    try {
        ...
        if (Array.isArray(saved.photos)) {
            photos = saved.photos
                ...
                .map(p => ({
                    id: typeof p.id === "string" ? p.id : String(p.time),
                    lat: p.lat, lng: p.lng,
        ...
        compactPathData();
        await migrateInlinePhotoAssets();
        await hydratePhotosFromDb();
...
async function handlePhotos(event) {
    ...
    const settled = await Promise.allSettled(files.map(processPhotoFile));
    const createdPhotos = settled
        .filter(r => r.status === "fulfilled" && r.value)
        .map(r => r.value);
    createdPhotos.forEach((photo, index) => {
        photos.push(photo);
        createPhotoMarker(photo, index === createdPhotos.length - 1);
    });
...
// ✅ pane: "memoryPane" 추가 — 사진 마커가 안개 위에 표시됨
function createPhotoMarker(data, openPopup = false) {
    const size = getPhotoMarkerSize();
    const marker = L.marker([data.lat, data.lng], {
        pane: "memoryPane",
        icon: createPhotoIcon(data.thumb, size)
    });
...
function deletePhoto(id) {
    ...
    if (marker) { photoClusterGroup.removeLayer(marker); photoMarkers.delete(id); }
    void deletePhotoAsset(id);
...
async function processPhotoFile(file) {
    ...
    const now = new Date();
    const fallback = currentPos || map.getCenter();
    const photo = {
        id: String(now.getTime()) + Math.random().toString(36).slice(2),
        lat: coords.lat ?? fallback.lat,
        lng: coords.lng ?? fallback.lng,
        photo: resizeImage(image, PHOTO_POPUP_SIZE),
        thumb: resizeImage(image, PHOTO_THUMB_SIZE),
        time: now.getTime(),
        dateString: now.toLocaleDateString("ko-KR", { year:"numeric", month:"long", day:"numeric" }),
        timeString: now.toLocaleTimeString("ko-KR", { hour:"2-digit", minute:"2-digit" })
    };
...
function extractPhotoCoordinates(file) {
    return new Promise(resolve => {
        EXIF.getData(file, function() {
            ...
            if (!latVal || !lngVal) { resolve({ lat: null, lng: null }); return; }
...
async function init() {
    attachEffectCanvasesToMap(); // ✅ 캔버스를 지도에 먼저 붙임
    resizeCanvas();
    await loadState();
    renderStoredMarkers();
    ...
    initGpxDial();
    initHudTapTargets();
    currentPhotoMarkerSize = getPhotoMarkerSize();
}

map.whenReady(() => { void init(); });
