...
function syncFogButton() {
    ...
    if (isFogEnabled) {
        ...
        if (toggleState) { toggleState.textContent = "켜짐"; toggleState.classList.add("on"); toggleState.classList.remove("off"); }
    } else {
        toggleBtn.classList.add("off"); toggleBtn.classList.remove("on");
        if (toggleState) { toggleState.textContent = "꺼짐"; toggleState.classList.add("off"); toggleState.classList.remove("on"); }
    }
}
...
function updateStats() {
    ...
    document.getElementById("today-dist-val").innerHTML = `${(today / 1000).toFixed(2)}<span>km</span>`;
    ...
    while (offset < view.byteLength) {
        ...
        if (marker === 0xFFE1) {
            ...
            for (let i = 0; i < ifdCount; i++) {
                ...
                if (getUint16(entryOffset) === 0x8825) gpsIfdOffset = getUint32(entryOffset + 8);
            }
            ...
            for (let i = 0; i < gpsCount; i++) {
                ...
                if (tag === 2 || tag === 4) {
                    const d = view.getUint32(tiffOffset + valOffset, littleEndian) / view.getUint32(tiffOffset + valOffset + 4, littleEndian);
                    const m = view.getUint32(tiffOffset + valOffset + 8, littleEndian) / view.getUint32(tiffOffset + valOffset + 12, littleEndian);
                    const s = view.getUint32(tiffOffset + valOffset + 16, littleEndian) / view.getUint32(tiffOffset + valOffset + 20, littleEndian);
    ...
    arrayReader.onload = function(ae) {
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                const data = {
                    id: String(now.getTime()),
                    lat: lat, lng: lng, photo: compressed,
                    time: now.getTime(),
                    dateString: now.toLocaleDateString("ko-KR", { year: "numeric", month: "long", day: "numeric" }),
                    timeString: now.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })
                };
...
function createPhotoMarker(data, openPopup = false) {
    ...
    const marker = L.marker([data.lat, data.lng], { pane: "memoryPane", icon: icon }).addTo(map);
    marker.bindPopup(
