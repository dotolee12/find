...
// ✅ 수정 1: 경로 없어도 안개가 전체를 덮음 (return 제거)
function renderFog() {
    ...
    if (pathCoordinates.length === 0) return; // 안개만 깔고 종료 (걷힐 경로 없음)
...
function updateHud() {
    ...
    const titleEl = ui.hudTitleText;
    const levelEl = ui.hudLevelNum;
    if (titleEl) titleEl.textContent = current.title;
    if (levelEl) levelEl.textContent = current.level;
    const distCurEl  = ui.progDistCur;
    const distBarEl  = ui.progDistBar;
    const distNextEl = ui.progDistNext;
    ...
    const memCurEl  = ui.progMemCur;
    const memBarEl  = ui.progMemBar;
    const memNextEl = ui.progMemNext;
    ...
    const photoCurEl  = ui.progPhotoCur;
    const photoBarEl  = ui.progPhotoBar;
    const photoNextEl = ui.progPhotoNext;
...
function updateStats() {
    ...
    const distEl  = ui.distVal;
    const todayEl = ui.todayDistVal;
    const memEl   = ui.memoryCountVal;
    const photoEl = ui.photoCountVal;
...
function findPhotoMarker(id) {
    return photoMarkers.get(id) || null;
}
...
// ✅ 수정 2: pane: "memoryPane" 추가 → 사진 마커가 안개 위에 표시됨
